...
func (r *StockBalanceRepository) Get(ctx context.Context, id string) (*model.StockBalance, error) {
	query := `SELECT balance_id, warehouse_id, location_id, material_id, quantity_on_hand, quantity_reserved, quantity_in_transit, quantity_available, last_count_date, created_by, updated_by, deleted_by, created_at, updated_at, deleted_at FROM inv_stock_balances WHERE balance_id = $1 AND deleted_at IS NULL`

	var m model.StockBalance
	err := r.DB.QueryRow(ctx, query, id).Scan(&m.BalanceId, &m.WarehouseId, &m.LocationId, &m.MaterialId, &m.QuantityOnHand, &m.QuantityReserved, &m.QuantityInTransit, &m.QuantityAvailable, &m.LastCountDate, &m.CreatedBy, &m.UpdatedBy, &m.DeletedBy, &m.CreatedAt, &m.UpdatedAt, &m.DeletedAt)
	if err != nil {
		return nil, err
	}
...
func (r *StockBalanceRepository) List(ctx context.Context, opts model.ListOptions) ([]model.StockBalance, int, error) {
	...
	countQuery := fmt.Sprintf("SELECT COUNT(*) FROM inv_stock_balances WHERE %s", whereSql)
	var total int
	...
	listQuery := fmt.Sprintf("SELECT balance_id, warehouse_id, location_id, material_id, quantity_on_hand, quantity_reserved, quantity_in_transit, quantity_available, last_count_date, created_by, updated_by, deleted_by, created_at, updated_at, deleted_at FROM inv_stock_balances WHERE %s ORDER BY created_at DESC LIMIT $%d OFFSET $%d", whereSql, argPos, argPos+1)
	...
	for rows.Next() {
		var m model.StockBalance
		if err := rows.Scan(&m.BalanceId, &m.WarehouseId, &m.LocationId, &m.MaterialId, &m.QuantityOnHand, &m.QuantityReserved, &m.QuantityInTransit, &m.QuantityAvailable, &m.LastCountDate, &m.CreatedBy, &m.UpdatedBy, &m.DeletedBy, &m.CreatedAt, &m.UpdatedAt, &m.DeletedAt); err != nil {
			return nil, 0, err
		}
