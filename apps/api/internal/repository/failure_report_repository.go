...
func (r *FailureReportRepository) Get(ctx context.Context, id string) (*model.FailureReport, error) {
	query := `SELECT report_id, equipment_id, reported_by_user_id, report_number, incident_date, severity, failure_mode, description, operational_impact, immediate_action_taken, status, created_by, updated_by, deleted_by, created_at, updated_at, deleted_at FROM mro_failure_reports WHERE report_id = $1 AND deleted_at IS NULL`

	var m model.FailureReport
	err := r.DB.QueryRow(ctx, query, id).Scan(&m.ReportId, &m.EquipmentId, &m.ReportedByUserId, &m.ReportNumber, &m.IncidentDate, &m.Severity, &m.FailureMode, &m.Description, &m.OperationalImpact, &m.ImmediateActionTaken, &m.Status, &m.CreatedBy, &m.UpdatedBy, &m.DeletedBy, &m.CreatedAt, &m.UpdatedAt, &m.DeletedAt)
	if err != nil {
		return nil, err
	}
...
func (r *FailureReportRepository) List(ctx context.Context, opts model.ListOptions) ([]model.FailureReport, int, error) {
	...
	countQuery := fmt.Sprintf("SELECT COUNT(*) FROM mro_failure_reports WHERE %s", whereSql)
	var total int
	...
	listQuery := fmt.Sprintf("SELECT report_id, equipment_id, reported_by_user_id, report_number, incident_date, severity, failure_mode, description, operational_impact, immediate_action_taken, status, created_by, updated_by, deleted_by, created_at, updated_at, deleted_at FROM mro_failure_reports WHERE %s ORDER BY created_at DESC LIMIT $%d OFFSET $%d", whereSql, argPos, argPos+1)
	...
	for rows.Next() {
		var m model.FailureReport
		if err := rows.Scan(&m.ReportId, &m.EquipmentId, &m.ReportedByUserId, &m.ReportNumber, &m.IncidentDate, &m.Severity, &m.FailureMode, &m.Description, &m.OperationalImpact, &m.ImmediateActionTaken, &m.Status, &m.CreatedBy, &m.UpdatedBy, &m.DeletedBy, &m.CreatedAt, &m.UpdatedAt, &m.DeletedAt); err != nil {
			return nil, 0, err
		}
