...
func (r *ShipRepository) Get(ctx context.Context, id string) (*model.Ship, error) {
	query := `SELECT ship_id, class_id, assigned_unit_id, hull_number, ship_name, call_sign, commission_date, home_port, length_m, beam_m, draft_m, displacement_tons, max_speed_knots, cruise_range_nm, crew_capacity, fuel_capacity_liters, fresh_water_capacity_liters, status, current_readiness_status, created_by, updated_by, deleted_by, created_at, updated_at, deleted_at FROM mro_ships WHERE ship_id = $1 AND deleted_at IS NULL`

	var m model.Ship
	err := r.DB.QueryRow(ctx, query, id).Scan(&m.ShipId, &m.ClassId, &m.AssignedUnitId, &m.HullNumber, &m.ShipName, &m.CallSign, &m.CommissionDate, &m.HomePort, &m.LengthM, &m.BeamM, &m.DraftM, &m.DisplacementTons, &m.MaxSpeedKnots, &m.CruiseRangeNm, &m.CrewCapacity, &m.FuelCapacityLiters, &m.FreshWaterCapacityLiters, &m.Status, &m.CurrentReadinessStatus, &m.CreatedBy, &m.UpdatedBy, &m.DeletedBy, &m.CreatedAt, &m.UpdatedAt, &m.DeletedAt)
	if err != nil {
		return nil, err
	}
...
func (r *ShipRepository) List(ctx context.Context, opts model.ListOptions) ([]model.Ship, int, error) {
	...
	if opts.Search != "" {
		...
		whereClauses = append(whereClauses, fmt.Sprintf("(hull_number ILIKE $%[1]d OR ship_name ILIKE $%[1]d OR call_sign ILIKE $%[1]d)", argPos))
		args = append(args, searchPattern)
		argPos++
	}

	whereSql := strings.Join(whereClauses, " AND ")
	countQuery := fmt.Sprintf("SELECT COUNT(*) FROM mro_ships WHERE %s", whereSql)
	var total int
	...
	listQuery := fmt.Sprintf("SELECT ship_id, class_id, assigned_unit_id, hull_number, ship_name, call_sign, commission_date, home_port, length_m, beam_m, draft_m, displacement_tons, max_speed_knots, cruise_range_nm, crew_capacity, fuel_capacity_liters, fresh_water_capacity_liters, status, current_readiness_status, created_by, updated_by, deleted_by, created_at, updated_at, deleted_at FROM mro_ships WHERE %s ORDER BY created_at DESC LIMIT $%d OFFSET $%d", whereSql, argPos, argPos+1)
	...
	for rows.Next() {
		var m model.Ship
		if err := rows.Scan(&m.ShipId, &m.ClassId, &m.AssignedUnitId, &m.HullNumber, &m.ShipName, &m.CallSign, &m.CommissionDate, &m.HomePort, &m.LengthM, &m.BeamM, &m.DraftM, &m.DisplacementTons, &m.MaxSpeedKnots, &m.CruiseRangeNm, &m.CrewCapacity, &m.FuelCapacityLiters, &m.FreshWaterCapacityLiters, &m.Status, &m.CurrentReadinessStatus, &m.CreatedBy, &m.UpdatedBy, &m.DeletedBy, &m.CreatedAt, &m.UpdatedAt, &m.DeletedAt); err != nil {
			return nil, 0, err
		}
