...
func NewCargoTypeRepository(db *pgxpool.Pool) *CargoTypeRepository {
	return &CargoTypeRepository{DB: db}
}

// ==================================================
// Create CargoType
// ==================================================
func (r *CargoTypeRepository) Create(ctx context.Context, m *model.CargoType) (int, error) {
	query := `
		...
		RETURNING cargo_type_id
	`

	var id int
	err := r.DB.QueryRow(ctx, query,
		m.CargoTypeName,
		m.CargoTypeDescription,
		m.CargoTypeGrade,
		m.IsActive,
		m.CreatedBy,
		m.UpdatedBy,
...
// Update CargoType
// ==================================================
func (r *CargoTypeRepository) Update(ctx context.Context, id int, m *model.CargoType) error {

	setIsActive := ""
	args := []interface{}{
		m.CargoTypeName,        // $1
		m.CargoTypeDescription, // $2
		m.CargoTypeGrade,       // $3
	}
	argPos := len(args) + 1
	...
	query := fmt.Sprintf(`
		UPDATE cargo_type
		SET cargo_type_name = $1,
			cargo_type_description = $2,
			cargo_type_grade = $3
			%s,
			updated_by = $%d,
			updated_at = $%d
		WHERE cargo_type_id = $%d AND deleted_at IS NULL
	`,
		setIsActive,
		argPos,   // updated_by
		argPos+1, // updated_at
...
// ==================================================
// Soft Delete CargoType
// ==================================================
func (r *CargoTypeRepository) SoftDelete(ctx context.Context, id, deletedBy int) error {
	query := `
		UPDATE cargo_type
		SET deleted_at = $1,
			deleted_by = $2,
			...
			updated_at = $1,
			updated_by = $2
		WHERE cargo_type_id = $3 AND deleted_at IS NULL
	`
...
// Get By ID
// ==================================================
func (r *CargoTypeRepository) GetByID(ctx context.Context, id int) (*model.CargoType, error) {
	query := `
		SELECT 
			cargo_type_id,
			cargo_type_name,
			...
			deleted_at
		FROM cargo_type
		WHERE cargo_type_id = $1 AND deleted_at IS NULL
	`

	var m model.CargoType
...
// ==================================================
func (r *CargoTypeRepository) List(
	ctx context.Context,
	cursorValue interface{}, cursorKey *int, limit int, filters map[string]string,
	orderBy, sort string,
) ([]model.CargoType, error) {
	tableKey := "cargo_type_id"
	...
	likeFilters := map[string]string{
		"cargo_type_name":        "cargo_type_name ILIKE $%d",
		"cargo_type_description": "cargo_type_description ILIKE $%d",
		"cargo_type_grade":       "cargo_type_grade ILIKE $%d",
	}
