...
func NewTenderHandler(s *service.TenderService, c *config.Config) *TenderHandler {
	return &TenderHandler{Svc: s, Cfg: c}
}

func (h *TenderHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.TenderRequest
	...
	response.JSON(w, http.StatusCreated, "tender created", item, nil)
}
...
func (h *TenderHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	response.JSON(w, http.StatusOK, "tender updated", item, nil)
}
...
func (h *TenderHandler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	response.JSON(w, http.StatusOK, "tender deleted", nil, nil)
}
