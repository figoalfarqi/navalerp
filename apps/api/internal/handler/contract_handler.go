...
func NewContractHandler(s *service.ContractService, c *config.Config) *ContractHandler {
	return &ContractHandler{Svc: s, Cfg: c}
}

func (h *ContractHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.ContractRequest
	...
	response.JSON(w, http.StatusCreated, "contract created", item, nil)
}
...
func (h *ContractHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	response.JSON(w, http.StatusOK, "contract updated", item, nil)
}
...
func (h *ContractHandler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	response.JSON(w, http.StatusOK, "contract deleted", nil, nil)
}
