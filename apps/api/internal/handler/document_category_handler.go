...
type DocumentCategoryHandler struct {
	Svc *service.DocumentCategoryService
	Cfg *config.Config
}

func NewDocumentCategoryHandler(s *service.DocumentCategoryService, c *config.Config) *DocumentCategoryHandler {
	return &DocumentCategoryHandler{Svc: s, Cfg: c}
}

func (h *DocumentCategoryHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.DocumentCategoryRequest
	...
	response.JSON(w, http.StatusCreated, "document_category created", item, nil)
}

func (h *DocumentCategoryHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id != "" {
...
func (h *DocumentCategoryHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "document_category updated", item, nil)
}

func (h *DocumentCategoryHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "document_category deleted", nil, nil)
}
