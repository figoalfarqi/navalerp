...
type TransportUnitHandler struct {
	Svc *service.TransportUnitService
	Cfg *config.Config
}

func NewTransportUnitHandler(s *service.TransportUnitService, c *config.Config) *TransportUnitHandler {
	return &TransportUnitHandler{Svc: s, Cfg: c}
}

func (h *TransportUnitHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.TransportUnitRequest
	...
	response.JSON(w, http.StatusCreated, "transport_unit created", item, nil)
}

func (h *TransportUnitHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id != "" {
...
func (h *TransportUnitHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "transport_unit updated", item, nil)
}

func (h *TransportUnitHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "transport_unit deleted", nil, nil)
}
