...
func NewStockAdjustmentHandler(s *service.StockAdjustmentService, c *config.Config) *StockAdjustmentHandler {
	return &StockAdjustmentHandler{Svc: s, Cfg: c}
}
...
func (h *StockAdjustmentHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	response.JSON(w, http.StatusCreated, "stock_adjustment created", item, nil)
}

func (h *StockAdjustmentHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id != "" {
...
func (h *StockAdjustmentHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "stock_adjustment updated", item, nil)
}

func (h *StockAdjustmentHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "stock_adjustment deleted", nil, nil)
}
