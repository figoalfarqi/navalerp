...
type ReadinessReportHandler struct {
	Svc *service.ReadinessReportService
	Cfg *config.Config
}

func NewReadinessReportHandler(s *service.ReadinessReportService, c *config.Config) *ReadinessReportHandler {
	return &ReadinessReportHandler{Svc: s, Cfg: c}
}

func (h *ReadinessReportHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.ReadinessReportRequest
	...
	response.JSON(w, http.StatusCreated, "readiness_report created", item, nil)
}

func (h *ReadinessReportHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id != "" {
...
func (h *ReadinessReportHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "readiness_report updated", item, nil)
}

func (h *ReadinessReportHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "readiness_report deleted", nil, nil)
}
