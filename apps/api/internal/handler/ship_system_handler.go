...
type ShipSystemHandler struct {
	Svc *service.ShipSystemService
	Cfg *config.Config
}

func NewShipSystemHandler(s *service.ShipSystemService, c *config.Config) *ShipSystemHandler {
	return &ShipSystemHandler{Svc: s, Cfg: c}
}

func (h *ShipSystemHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.ShipSystemRequest
	...
	response.JSON(w, http.StatusCreated, "ship_system created", item, nil)
}

func (h *ShipSystemHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id != "" {
...
func (h *ShipSystemHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "ship_system updated", item, nil)
}

func (h *ShipSystemHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "ship_system deleted", nil, nil)
}
