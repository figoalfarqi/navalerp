...
type PlatformTcoHandler struct {
	Svc *service.PlatformTcoService
	Cfg *config.Config
}

func NewPlatformTcoHandler(s *service.PlatformTcoService, c *config.Config) *PlatformTcoHandler {
	return &PlatformTcoHandler{Svc: s, Cfg: c}
}

func (h *PlatformTcoHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.PlatformTcoRequest
	...
	response.JSON(w, http.StatusCreated, "platform_tco created", item, nil)
}

func (h *PlatformTcoHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id != "" {
...
func (h *PlatformTcoHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "platform_tco updated", item, nil)
}

func (h *PlatformTcoHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "platform_tco deleted", nil, nil)
}
