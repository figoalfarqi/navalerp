...
type ShipClassHandler struct {
	Svc *service.ShipClassService
	Cfg *config.Config
}

func NewShipClassHandler(s *service.ShipClassService, c *config.Config) *ShipClassHandler {
	return &ShipClassHandler{Svc: s, Cfg: c}
}

func (h *ShipClassHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.ShipClassRequest
	...
	response.JSON(w, http.StatusCreated, "ship_class created", item, nil)
}

func (h *ShipClassHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id != "" {
...
func (h *ShipClassHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "ship_class updated", item, nil)
}

func (h *ShipClassHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "ship_class deleted", nil, nil)
}
