...
type ChartOfAccountHandler struct {
	Svc *service.ChartOfAccountService
	Cfg *config.Config
}

func NewChartOfAccountHandler(s *service.ChartOfAccountService, c *config.Config) *ChartOfAccountHandler {
	return &ChartOfAccountHandler{Svc: s, Cfg: c}
}

func (h *ChartOfAccountHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.ChartOfAccountRequest
	...
	response.JSON(w, http.StatusCreated, "chart_of_account created", item, nil)
}

func (h *ChartOfAccountHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id != "" {
...
func (h *ChartOfAccountHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "chart_of_account updated", item, nil)
}

func (h *ChartOfAccountHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "chart_of_account deleted", nil, nil)
}
