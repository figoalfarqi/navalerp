...
type MilitaryCorpsHandler struct {
	Svc *service.MilitaryCorpsService
	Cfg *config.Config
}

func NewMilitaryCorpsHandler(s *service.MilitaryCorpsService, c *config.Config) *MilitaryCorpsHandler {
	return &MilitaryCorpsHandler{Svc: s, Cfg: c}
}

func (h *MilitaryCorpsHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.MilitaryCorpsRequest
	...
	response.JSON(w, http.StatusCreated, "military_corps created", item, nil)
}

func (h *MilitaryCorpsHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id != "" {
...
func (h *MilitaryCorpsHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "military_corps updated", item, nil)
}

func (h *MilitaryCorpsHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "military_corps deleted", nil, nil)
}
