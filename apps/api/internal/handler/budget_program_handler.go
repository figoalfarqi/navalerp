...
type BudgetProgramHandler struct {
	Svc *service.BudgetProgramService
	Cfg *config.Config
}

func NewBudgetProgramHandler(s *service.BudgetProgramService, c *config.Config) *BudgetProgramHandler {
	return &BudgetProgramHandler{Svc: s, Cfg: c}
}

func (h *BudgetProgramHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.BudgetProgramRequest
	...
	response.JSON(w, http.StatusCreated, "budget_program created", item, nil)
}

func (h *BudgetProgramHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id != "" {
...
func (h *BudgetProgramHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "budget_program updated", item, nil)
}

func (h *BudgetProgramHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "budget_program deleted", nil, nil)
}
