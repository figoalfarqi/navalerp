...
func NewRequisitionHandler(s *service.RequisitionService, c *config.Config) *RequisitionHandler {
	return &RequisitionHandler{Svc: s, Cfg: c}
}
...
func (h *RequisitionHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	var req model.RequisitionRequest
	...
	response.JSON(w, http.StatusCreated, "requisition created", item, nil)
}
...
func (h *RequisitionHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "requisition updated", item, nil)
}

func (h *RequisitionHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "requisition deleted", nil, nil)
}
