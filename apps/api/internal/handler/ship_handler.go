...
func NewShipHandler(s *service.ShipService, c *config.Config) *ShipHandler {
	return &ShipHandler{Svc: s, Cfg: c}
}

func (h *ShipHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.ShipRequest
	...
	response.JSON(w, http.StatusCreated, "ship created", item, nil)
}

func (h *ShipHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id != "" {
...
func (h *ShipHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "ship updated", item, nil)
}

func (h *ShipHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "ship deleted", nil, nil)
}
