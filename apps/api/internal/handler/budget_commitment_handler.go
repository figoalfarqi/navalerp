...
type BudgetCommitmentHandler struct {
	Svc *service.BudgetCommitmentService
	Cfg *config.Config
}

func NewBudgetCommitmentHandler(s *service.BudgetCommitmentService, c *config.Config) *BudgetCommitmentHandler {
	return &BudgetCommitmentHandler{Svc: s, Cfg: c}
}

func (h *BudgetCommitmentHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.BudgetCommitmentRequest
	...
	response.JSON(w, http.StatusCreated, "budget_commitment created", item, nil)
}

func (h *BudgetCommitmentHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id != "" {
...
func (h *BudgetCommitmentHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "budget_commitment updated", item, nil)
}

func (h *BudgetCommitmentHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "budget_commitment deleted", nil, nil)
}
