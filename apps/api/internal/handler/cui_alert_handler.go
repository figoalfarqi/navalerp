...
type CuiAlertHandler struct {
	Svc *service.CuiAlertService
	Cfg *config.Config
}

func NewCuiAlertHandler(svc *service.CuiAlertService, cfg *config.Config) *CuiAlertHandler {
	return &CuiAlertHandler{Svc: svc, Cfg: cfg}
}

func (h *CuiAlertHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id != "" {
		item, err := h.Svc.GetByID(r.Context(), id)
		if err != nil {
			response.JSON(w, http.StatusNotFound, "cui_alert not found", nil, nil)
			return
		}
		response.JSON(w, http.StatusOK, "cui_alert retrieved", item, nil)
		return
	}
	...
	response.JSON(w, http.StatusOK, "cui_alert list retrieved", map[string]any{
		"items": items,
		"total": total,
		"page":  page,
		"limit": limit,
...
func (h *CuiAlertHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.CuiAlertRequest
	...
	response.JSON(w, http.StatusCreated, "cui_alert created", item, nil)
}

func (h *CuiAlertHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "cui_alert updated", item, nil)
}

func (h *CuiAlertHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "cui_alert deleted", nil, nil)
}
