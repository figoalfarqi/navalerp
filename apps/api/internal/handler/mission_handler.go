...
func NewMissionHandler(s *service.MissionService, c *config.Config) *MissionHandler {
	return &MissionHandler{Svc: s, Cfg: c}
}

func (h *MissionHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.MissionRequest
	...
	response.JSON(w, http.StatusCreated, "mission created", item, nil)
}
...
func (h *MissionHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	response.JSON(w, http.StatusOK, "mission updated", item, nil)
}
...
func (h *MissionHandler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	response.JSON(w, http.StatusOK, "mission deleted", nil, nil)
}
