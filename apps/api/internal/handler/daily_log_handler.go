...
type DailyLogHandler struct {
	Svc *service.DailyLogService
	Cfg *config.Config
}

func NewDailyLogHandler(s *service.DailyLogService, c *config.Config) *DailyLogHandler {
	return &DailyLogHandler{Svc: s, Cfg: c}
}
...
func (h *DailyLogHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	response.JSON(w, http.StatusCreated, "daily_log created", item, nil)
}

func (h *DailyLogHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id != "" {
...
func (h *DailyLogHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	response.JSON(w, http.StatusOK, "daily_log updated", item, nil)
}
...
func (h *DailyLogHandler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	response.JSON(w, http.StatusOK, "daily_log deleted", nil, nil)
}
