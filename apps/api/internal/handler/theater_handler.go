...
type TheaterHandler struct {
	Svc *service.TheaterService
	Cfg *config.Config
}

func NewTheaterHandler(s *service.TheaterService, c *config.Config) *TheaterHandler {
	return &TheaterHandler{Svc: s, Cfg: c}
}

func (h *TheaterHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.TheaterRequest
	...
	response.JSON(w, http.StatusCreated, "theater created", item, nil)
}
...
func (h *TheaterHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "theater updated", item, nil)
}

func (h *TheaterHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "theater deleted", nil, nil)
}
