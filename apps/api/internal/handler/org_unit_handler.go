...
func NewOrgUnitHandler(s *service.OrgUnitService, c *config.Config) *OrgUnitHandler {
	return &OrgUnitHandler{Svc: s, Cfg: c}
}

func (h *OrgUnitHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.OrgUnitRequest
	...
	response.JSON(w, http.StatusCreated, "org_unit created", item, nil)
}

func (h *OrgUnitHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id != "" {
...
func (h *OrgUnitHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "org_unit updated", item, nil)
}

func (h *OrgUnitHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "org_unit deleted", nil, nil)
}
