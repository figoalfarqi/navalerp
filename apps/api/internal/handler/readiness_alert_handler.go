...
type ReadinessAlertHandler struct {
	Svc *service.ReadinessAlertService
	Cfg *config.Config
}

func NewReadinessAlertHandler(s *service.ReadinessAlertService, c *config.Config) *ReadinessAlertHandler {
	return &ReadinessAlertHandler{Svc: s, Cfg: c}
}

func (h *ReadinessAlertHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.ReadinessAlertRequest
	...
	response.JSON(w, http.StatusCreated, "readiness_alert created", item, nil)
}

func (h *ReadinessAlertHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id != "" {
...
func (h *ReadinessAlertHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "readiness_alert updated", item, nil)
}

func (h *ReadinessAlertHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "readiness_alert deleted", nil, nil)
}
