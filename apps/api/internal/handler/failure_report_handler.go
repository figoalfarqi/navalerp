...
type FailureReportHandler struct {
	Svc *service.FailureReportService
	Cfg *config.Config
}

func NewFailureReportHandler(s *service.FailureReportService, c *config.Config) *FailureReportHandler {
	return &FailureReportHandler{Svc: s, Cfg: c}
}

func (h *FailureReportHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.FailureReportRequest
	...
	response.JSON(w, http.StatusCreated, "failure_report created", item, nil)
}

func (h *FailureReportHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id != "" {
...
func (h *FailureReportHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "failure_report updated", item, nil)
}

func (h *FailureReportHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "failure_report deleted", nil, nil)
}
