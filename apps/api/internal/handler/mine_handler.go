...
type MineHandler struct {
	Svc *service.MineService
	Cfg *config.Config
}

func NewMineHandler(s *service.MineService, c *config.Config) *MineHandler {
	return &MineHandler{Svc: s, Cfg: c}
}

// ==================================================
// POST /api/v1/admin/mine
// ==================================================
func (h *MineHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	response.JSON(w, http.StatusCreated, "mine created", cldst, nil)
}

// ==================================================
// PUT /api/v1/admin/mine/{id}
// ==================================================
func (h *MineHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	_, id := helper.GetIDAndRoleFromPath(r.URL.Path, "mine")
	if id <= 0 {
		response.JSON(
			w,
	...
	response.JSON(w, http.StatusOK, "mine updated", cldst, nil)
}

// ==================================================
// DELETE /api/v1/admin/mine/{id}
// ==================================================
func (h *MineHandler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.Svc.Delete(r.Context(), loginID, id); err != nil {
		response.JSON(w, http.StatusInternalServerError, "failed to delete mine", nil,
			map[string]string{"error": err.Error()},
		)
		return
	}

	response.JSON(w, http.StatusOK, "mine deleted", map[string]int{"mine_id": id}, nil)
}

// ==================================================
// GET /api/v1/admin/mine OR /{id}
// ==================================================
func (h *MineHandler) Get(w http.ResponseWriter, r *http.Request) {
	...
	if id > 0 {
		...
		if err != nil {
			if errors.Is(err, pgx.ErrNoRows) {
				response.JSON(w, http.StatusNotFound, "mine not found", nil, nil)
				return
			}
			response.JSON(
				w,
				http.StatusInternalServerError,
				"failed to fetch mine",
				nil,
				map[string]string{"error": err.Error()},
			)
			return
	...
	if err != nil {
		response.JSON(w, http.StatusInternalServerError, "failed to list mines", nil,
			map[string]string{"error": err.Error()},
		)
		return
	}
