...
func NewStockTransferHandler(s *service.StockTransferService, c *config.Config) *StockTransferHandler {
	return &StockTransferHandler{Svc: s, Cfg: c}
}
...
func (h *StockTransferHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	response.JSON(w, http.StatusCreated, "stock_transfer created", item, nil)
}
...
func (h *StockTransferHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "stock_transfer updated", item, nil)
}

func (h *StockTransferHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "stock_transfer deleted", nil, nil)
}
