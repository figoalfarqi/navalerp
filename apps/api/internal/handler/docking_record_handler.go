...
type DockingRecordHandler struct {
	Svc *service.DockingRecordService
	Cfg *config.Config
}

func NewDockingRecordHandler(s *service.DockingRecordService, c *config.Config) *DockingRecordHandler {
	return &DockingRecordHandler{Svc: s, Cfg: c}
}

func (h *DockingRecordHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.DockingRecordRequest
	...
	response.JSON(w, http.StatusCreated, "docking_record created", item, nil)
}

func (h *DockingRecordHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id != "" {
...
func (h *DockingRecordHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "docking_record updated", item, nil)
}

func (h *DockingRecordHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "docking_record deleted", nil, nil)
}
