...
type BaseFacilityHandler struct {
	Svc *service.BaseFacilityService
	Cfg *config.Config
}

func NewBaseFacilityHandler(s *service.BaseFacilityService, c *config.Config) *BaseFacilityHandler {
	return &BaseFacilityHandler{Svc: s, Cfg: c}
}

func (h *BaseFacilityHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.BaseFacilityRequest
	...
	response.JSON(w, http.StatusCreated, "base_facility created", item, nil)
}

func (h *BaseFacilityHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id != "" {
...
func (h *BaseFacilityHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "base_facility updated", item, nil)
}

func (h *BaseFacilityHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "base_facility deleted", nil, nil)
}
