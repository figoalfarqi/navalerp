...
type BerthBookingHandler struct {
	Svc *service.BerthBookingService
	Cfg *config.Config
}

func NewBerthBookingHandler(s *service.BerthBookingService, c *config.Config) *BerthBookingHandler {
	return &BerthBookingHandler{Svc: s, Cfg: c}
}

func (h *BerthBookingHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.BerthBookingRequest
	...
	response.JSON(w, http.StatusCreated, "berth_booking created", item, nil)
}

func (h *BerthBookingHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id != "" {
...
func (h *BerthBookingHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "berth_booking updated", item, nil)
}

func (h *BerthBookingHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "berth_booking deleted", nil, nil)
}
