...
type PmScheduleHandler struct {
	Svc *service.PmScheduleService
	Cfg *config.Config
}

func NewPmScheduleHandler(s *service.PmScheduleService, c *config.Config) *PmScheduleHandler {
	return &PmScheduleHandler{Svc: s, Cfg: c}
}

func (h *PmScheduleHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.PmScheduleRequest
	...
	response.JSON(w, http.StatusCreated, "pm_schedule created", item, nil)
}

func (h *PmScheduleHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id != "" {
...
func (h *PmScheduleHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "pm_schedule updated", item, nil)
}

func (h *PmScheduleHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "pm_schedule deleted", nil, nil)
}
