...
type CrewAssignmentHandler struct {
	Svc *service.CrewAssignmentService
	Cfg *config.Config
}

func NewCrewAssignmentHandler(s *service.CrewAssignmentService, c *config.Config) *CrewAssignmentHandler {
	return &CrewAssignmentHandler{Svc: s, Cfg: c}
}

func (h *CrewAssignmentHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.CrewAssignmentRequest
	...
	response.JSON(w, http.StatusCreated, "crew_assignment created", item, nil)
}

func (h *CrewAssignmentHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id != "" {
...
func (h *CrewAssignmentHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "crew_assignment updated", item, nil)
}

func (h *CrewAssignmentHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "crew_assignment deleted", nil, nil)
}
