...
func NewStockBalanceHandler(s *service.StockBalanceService, c *config.Config) *StockBalanceHandler {
	return &StockBalanceHandler{Svc: s, Cfg: c}
}

func (h *StockBalanceHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.StockBalanceRequest
	...
	response.JSON(w, http.StatusCreated, "stock_balance created", item, nil)
}

func (h *StockBalanceHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id != "" {
...
func (h *StockBalanceHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "stock_balance updated", item, nil)
}

func (h *StockBalanceHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "stock_balance deleted", nil, nil)
}
