...
func NewSysUserHandler(s *service.SysUserService, c *config.Config) *SysUserHandler {
	return &SysUserHandler{Svc: s, Cfg: c}
}

func (h *SysUserHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.SysUserRequest
	...
	response.JSON(w, http.StatusCreated, "sys_user created", item, nil)
}

func (h *SysUserHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id != "" {
...
func (h *SysUserHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "sys_user updated", item, nil)
}

func (h *SysUserHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "sys_user deleted", nil, nil)
}
