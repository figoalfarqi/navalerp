...
type FuelBunkerHandler struct {
	Svc *service.FuelBunkerService
	Cfg *config.Config
}

func NewFuelBunkerHandler(s *service.FuelBunkerService, c *config.Config) *FuelBunkerHandler {
	return &FuelBunkerHandler{Svc: s, Cfg: c}
}

func (h *FuelBunkerHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.FuelBunkerRequest
	...
	response.JSON(w, http.StatusCreated, "fuel_bunker created", item, nil)
}

func (h *FuelBunkerHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id != "" {
...
func (h *FuelBunkerHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
	...
	response.JSON(w, http.StatusOK, "fuel_bunker updated", item, nil)
}

func (h *FuelBunkerHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		response.JSON(w, http.StatusBadRequest, "id is required", nil, nil)
		return
	...
	response.JSON(w, http.StatusOK, "fuel_bunker deleted", nil, nil)
}
