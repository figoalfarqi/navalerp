...
func NewPersonnelHandler(s *service.PersonnelService, c *config.Config) *PersonnelHandler {
	return &PersonnelHandler{Svc: s, Cfg: c}
}

func (h *PersonnelHandler) Create(w http.ResponseWriter, r *http.Request) {
	loginID, _ := r.Context().Value(middleware.CtxUserID).(string)
	var req model.PersonnelRequest
	...
	response.JSON(w, http.StatusCreated, "personnel created", item, nil)
}

func (h *PersonnelHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id != "" {
...
func (h *PersonnelHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	response.JSON(w, http.StatusOK, "personnel updated", item, nil)
}
...
func (h *PersonnelHandler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	response.JSON(w, http.StatusOK, "personnel deleted", nil, nil)
}
